...
const buildNodes = (childKeys = [], data, level = 0) => {
  ...
  if (currentKey) {
    result = data[currentKey].map(item => {
      if (keys[0]) {
        newData[keys[0]] = [ ...newData[keys[0]], ...item[keys[0]].map(subItem => ({
          ...subItem,
...
const TopologyView = () => {
  ...
  const findAllChildren = (nodeIds, edges = []) => {
    ...
    let newIds = [];
    nodeIds.forEach(id => {
      allEdges.push(...state.edges.filter(({ source, target }) => {
        if (source === id) {
          newIds.push(target);
        }
  ...
  const handleNodeClick = (node) => {
    ...
    if (!node.wasClicked && node.children !== undefined) {
      ...
      })), ...state.structure[node.childKey].filter(({ parentId }) => node.id === parentId).map(item => ({
        mainGroup: node.mainGroup || node.originalId,
        ...item
      })) ];
      const edges = [ ...state.edges, ...state.structure[node.childKey]
